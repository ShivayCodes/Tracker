...
public class Chunk {
    ...
    private static final float TEX_WIDTH = 0.2f; // 5 tiles now
        ...
        for (int x = 2; x < SIZE - 2; x++) {
            for (int z = 2; z < SIZE - 2; z++) {
                for (int y = SIZE - 2; y >= 2; y--) {
                    if (blocks[x][y][z] == 2) { // Grass
                        if (rand.nextFloat() > 0.98f) { // 2% chance
                            spawnTree(x, y + 1, z);
                        }
                        break;
                    }
    ...
    private void spawnTree(int x, int y, int z) {
        // Trunk (3-5 blocks high)
        for (int i = 0; i < 4; i++) {
            if (y + i < SIZE) blocks[x][y + i][z] = 4; // Wood
        }
        // Leaves (small canopy)
        for (int ix = -2; ix <= 2; ix++) {
            for (int iy = 2; iy <= 4; iy++) {
                for (int iz = -2; iz <= 2; iz++) {
                    if (Math.abs(ix) == 2 && Math.abs(iz) == 2) continue; // Round corners
                    if (x + ix >= 0 && x + ix < SIZE && y + iy >= 0 && y + iy < SIZE && z + iz >= 0 && z + iz < SIZE) {
                        if (blocks[x + ix][y + iy][z + iz] == 0) {
                            blocks[x + ix][y + iy][z + iz] = 5; // Leaves
                        }
                    }
                }
            }
    ...
    public void rebuildMesh() {
        ...
        float[] texArr = toFloatArray(textCoords);
        float[] normArr = toFloatArray(normals);
        float[] aoArr = toFloatArray(aoValues);
    ...
    private float calcAO(int x1, int y1, int z1, int x2, int y2, int z2, int x3, int y3, int z3) {
        ...
        if (side1 == 1 && side2 == 1) return 0.4f;
    ...
    private void addNormals(List<Float> normals, float nx, float ny, float nz) {
        for (int i = 0; i < 4; i++) {
            normals.add(nx); normals.add(ny); normals.add(nz);
        }
    }

    private void addTexCoords(List<Float> textCoords, float... coords) {
        for (float c : coords) textCoords.add(c);
    }
